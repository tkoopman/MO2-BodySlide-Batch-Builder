...
class Build(Output):
    ...
    def __init__(
        self, enable: bool, output: str, preset: str, include: list[IncludeItem]
    ):
    ...
    def __eq__(self, other: Any) -> bool:
        if isinstance(other, Build):
            return (
                self.output.casefold() == other.output.casefold()
                and self.enable == other.enable
                and self.preset == other.preset
                and len(self.include) == len(other.include)
                and all(
...
class Global(Output):
    ...
    def __init__(
        ...
        onBuildCheckConflicts: bool,
        onBuildCheckIgnored: bool,
        autoClose: bool,
        showSources: bool,
        output: str,
    ):
...
def convertToBool(value: Any, *, default: bool = False) -> bool:
    return (
        value.casefold() in ["true", "yes", "1"] if isinstance(value, str) else default
    )
...
def saveConfig(globalConfig: Global, builds: list[Build], file_path: str):
    ...
    config.append(
        ET.Element("Setting", name="deleteMeshes", value=str(globalConfig.deleteMeshes))
    )
    config.append(
        ET.Element(
            "Setting",
    ...
    config.append(
        ET.Element("Setting", name="autoClose", value=str(globalConfig.autoClose))
    )
    config.append(
    ...
    for build in builds:
        build_element = ET.Element(
            "Build", enable=str(build.enable), output=build.output, preset=build.preset
        )
        for include in build.include:
            if (
                include.type == IncludeType.SLIDERSET
                and include.use == IncludeUse.IncludeKeep
            ):
                e = ET.Element("Include", name=include.name)
            elif include.type == IncludeType.SLIDERSET:
                e = ET.Element(
                    "Include", name=include.name, use=includeUseToStr(include.use)
                )
            elif include.use == IncludeUse.IncludeKeep:
                e = ET.Element(
                    "Include", name=include.name, type=includeTypeToStr(include.type)
                )
            else:
                e = ET.Element(
                    "Include",
